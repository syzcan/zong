...
public class RandomCode {
	...
	public static void getRandcode(HttpServletRequest request, HttpServletResponse response) {
		...
		session.removeAttribute(ZConst.SESSION_SECURITY_CODE);
		session.setAttribute(ZConst.SESSION_SECURITY_CODE, randomString);
	...
	public static String getCode() {
		...
		for (int i = 1; i <= 4; i++) {
			code += String.valueOf(randString.charAt(random.nextInt(randString.length())));
		}
		return code;
	}
	...
	public static String getCode(int num) {
		...
		Random random = new Random();
		for (int i = 1; i <= num; i++) {
			code += randString.charAt(random.nextInt(randString.length()));
		}
		return code;
	}
	...
	public static String makeUserName() {
		String code = "";
		String cases = "abcdefghjkmnpqrstuvwxyz";
		Random random = new Random();
		for (int i = 1; i <= 2; i++) {
			code += cases.charAt(random.nextInt(cases.length()));
		}
		return code + getCode(6);
	}
